...
d = d.groupby(['scheduler', 'buffer_size']).agg({'time': [np.mean, np.var, conf_int]})

fig, ax = plt.subplots(1, 1)
fig.subplots_adjust(bottom=.192, left=.11, top=.99, right=.97)

t = d.loc[('smol1')].reset_index()
ax.errorbar(t['buffer_size'], t[('time', 'mean')], yerr=t[('time', 'conf_int')], label='Native')

t = d.loc[('wasm')].reset_index()
ax.errorbar(t['buffer_size'], t[('time', 'mean')], yerr=t[('time', 'conf_int')], label='WASM')
