...
fn get_commandline_args(run: &mut u64, scheduler : &mut String, items : &mut usize, buffersize : &mut u64){
    ...
    *run = value_t!(matches.value_of("run"), u64).context("no run").unwrap();
    *scheduler = value_t!(matches.value_of("scheduler"), String).context("no scheduler").unwrap();
    *items = value_t!(matches.value_of("items"), usize).context("no items").unwrap();
    *buffersize = value_t!(matches.value_of("buffersize"), u64).context("no buffersize").unwrap();
