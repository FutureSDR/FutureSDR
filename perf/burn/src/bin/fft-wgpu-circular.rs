...
impl Fft {
    fn new(instance: wgpu_buffer::Instance, args: Args) -> Result<Self> {
        ...
        input.set_min_items(args.batch_size * FFT_SIZE);
    ...
    fn create_state(
        ...
        let output_buffers = (0..READBACK_SLOTS)
            .map(|i| {
                device.create_buffer(&wgpu::BufferDescriptor {
                    label: Some(&format!("fft_readback_{i}")),
                    size: mag_bytes,
...
impl Fft {
    fn emit_one_pending(&mut self, pending: PendingReadback) -> Result<()> {
        let used_items;
        {
            ...
            if self.output.slice().len() < vals.len() {
                anyhow::bail!("not enough circular output space for WGPU readback");
            }
            self.output.slice()[..vals.len()].copy_from_slice(vals);
            used_items = vals.len();
        }
        pending.buffer.unmap();
        self.output.produce(used_items);
        self.output_items_emitted += used_items as u64;
        self.state
            .as_mut()
...
impl Kernel for Fft {
    async fn work(
        ...
        if self.input.slice().len() >= self.state.as_ref().unwrap().batch_size * FFT_SIZE {
            processed_input = true;
            let input_items = self.input.slice().len();
            self.input_items_reported += input_items as u64;
            ...
            let effective_items = input_items;
            ...
            if total_batches == 0 {
                warn!(
                    "fft-wgpu-circular: dropping invalid input buffer (input_items {}, expected_items {}, effective_items {}, max_batches_by_items {})",
                    input_items, expected_items, effective_items, max_batches_by_items
                );
            } else {
                ...
                for batch_start in (0..total_batches).step_by(state.chunk_batches) {
                    ...
                    if max_active_batches_by_dst_now == 0 {
                        warn!(
                            "fft-wgpu-circular: stopping chunk loop, destination buffer too small (dst_size_now {}) for one batch",
                            dst_size_now
                        );
                        break;
                    }
                    ...
                    let input_chunk = &self.input.slice()[start..end];
            ...
            self.input.consume(total_batches * FFT_SIZE);
        }

        if self.input.finished() {
            while self.emit_pending(true)? {}
            if self.state.as_ref().unwrap().pending_readbacks.is_empty() {
                eprintln!(
                    "fft-wgpu-circular counts: input_reported={} input_effective={} input_processed={} input_dropped={} output_emitted={}",
