...
pivot = g.pivot(index="frames", columns="sdr", values="time_mean").sort_index()
print(pivot)
...
fs = pivot["fs"].to_numpy() if "fs" in pivot.columns else np.zeros(len(frames))
gr = pivot["gr"].to_numpy() if "gr" in pivot.columns else np.zeros(len(frames))

fig, ax = plt.subplots(1, 1)
fig.subplots_adjust(bottom=0.2, left=0.12, top=0.98, right=0.98)

ax.bar(x - width / 2, fs, width, label="FutureSDR")
ax.bar(x + width / 2, gr, width, label="GNU Radio")
