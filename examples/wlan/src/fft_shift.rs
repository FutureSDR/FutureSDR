...
impl<T: Copy + Send + 'static> Kernel for FftShift<T> {
    async fn work(
        ...
        let n = std::cmp::min(input.len(), output.len()) / 64;

        for i in 0..n {
            for k in 0..64 {
                let m = (k + 32) % 64;
                output[i * 64 + m] = input[i * 64 + k]
            }
        }

        if sio.input(0).finished() && n == input.len() / 64 {
            io.finished = true;
        }

        sio.input(0).consume(n * 64);
        sio.output(0).produce(n * 64);
