...
impl WhisperBlock {
    fn new(model_path: &str, model_name: &str, language: Language, device: &Device<B>) -> Self {
        let (tokenizer, _config, model) = load_model::<B>(model_path, model_name, device);

        let n_mels = model.encoder_mel_size();
        let n_waveform_samples_per_window =
            max_waveform_samples(model.encoder_ctx_size() - PADDING) * 3 / 4;
    ...
    async fn work(
        ...
        for i in 0..iter_len {
            ...
            if let Some((prev_index, curr_index)) =
                find_chunk_overlap(&self.tokens[..], &new_tokens[..], 20, 3)
                && prev_index > 0
                && curr_index > 0
            {
...
struct Args {
    ...
    model_name: String,
    /// Model Path
    #[clap(long, default_value = "/home/basti/src/whisper-burn/tiny")]
    model_path: String,
    /// Language
    #[arg(long, value_enum, default_value_t = Language::German)]
    language: Language,
}
