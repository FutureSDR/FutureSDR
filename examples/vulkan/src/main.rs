...
fn run_cpu(orig: Vec<f32>) -> Result<Vec<f32>> {
    ...
    let snk = fg.block(&snk).unwrap();
    Ok(snk.items().clone())
}

fn run_vulkan(orig: Vec<f32>) -> Result<Vec<f32>> {
    let mut fg = Flowgraph::new();
    ...
    let mut src = VectorSource::<f32, H2DWriter<f32>>::new(orig);
    let vulkan = Vulkan::<f32>::new(instance.clone(), entry_point, 32);
...
fn main() -> Result<()> {
    ...
    let res = if args.cpu {
        ...
        run_vulkan(orig.clone())?
    };

    assert_eq!(res.len(), N_ITEMS);
    for i in 0..res.len() {
        assert!((orig[i].exp() - res[i]).abs() < 5.0 * f32::EPSILON);
    }
    Ok(())
}
