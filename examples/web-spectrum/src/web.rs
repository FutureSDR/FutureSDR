...
use crate::gpu::FFT_SIZE;
use crate::gpu::WgpuSpectrum;
...
pub fn Gui() -> impl IntoView {
    ...
    view! {
        <h1 class="m-4 text-xl text-white">FutureSDR WebGPU Spectrum</h1>
        {move || {
            match handle.get() {
                Some(handle) => {
...
async fn run(
    ...
    let spectrum = fg
        .with_local_domain_async(wgpu_domain, async move |ctx: &LocalDomainContext<'_>| {
            Ok(ctx.add(WgpuSpectrum::new().await))
        })
        .await?;
