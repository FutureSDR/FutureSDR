...
pub struct NormalDomainSpec {
    pub(crate) blocks: Vec<(BlockId, Box<dyn Block>)>,
...
impl NormalDomainSpec {
    ...
    /// Take the normal blocks out of this spec.
    pub(crate) fn into_parts(
        self,
    ) -> (
        Vec<(BlockId, Box<dyn Block>)>,
        DomainTopology,
...
pub struct NormalRunningDomain {
    tasks: Vec<Task<(BlockId, Box<dyn Block>)>>,
}

impl NormalRunningDomain {
    /// Create a running normal domain from block task handles.
    pub(crate) fn new(tasks: Vec<Task<(BlockId, Box<dyn Block>)>>) -> Self {
        Self { tasks }
    }

    /// Await all normal-domain block tasks and return their stopped blocks.
    pub(crate) async fn join(self) -> Result<Vec<(BlockId, Box<dyn Block>)>, Error> {
...
pub enum StoppedDomain {
    ...
    Normal(Vec<(BlockId, Box<dyn Block>)>),
