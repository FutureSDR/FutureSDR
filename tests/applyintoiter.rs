...
fn base_test(multiplier: usize, buf1_size: usize, buf2_size: usize) -> Result<()> {
    ...
    let apply_into_iter = fg.add_block(ApplyIntoIter::new(move |i: &f32| -> std::iter::Take<std::iter::Repeat<f32>> {
        std::iter::repeat(*i).take(multiplier.try_into().unwrap())
    }));
    let vect_sink = fg.add_block(VectorSinkBuilder::<f32>::new().build());

    fg.connect_stream_with_type(src, "out", apply_into_iter, "in", Circular::with_size(buf1_size))?;
    fg.connect_stream_with_type(apply_into_iter, "out", vect_sink, "in", Circular::with_size(buf2_size))?;
